...
def get_parser():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/s3dis_UPL_fs.yaml",
        help="config file",
    )
    parser.add_argument(
        "opts",
        help="see config/s3dis_UPL_fs.yaml for all options",
        default=None,
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train, avg_kl_loss_train = train(
            train_loader,
            model,
            optimizer,
            epoch,
        ...
        if main_process():
            ...
            writer.add_scalar("kl_loss_train", avg_kl_loss_train, epoch_log)

        is_best = False
        # 只在指定epoch及之后进行validate
        if args.evaluate and (epoch_log % args.eval_freq == 0) and (epoch_log >= args.eval_start_epoch):
            loss_val, mIoU_val, mAcc_val, allAcc_val, avg_uncertainty_val = validate(
            ...
            if main_process():
                ...
                writer.add_scalar("avg_uncertainty_val", avg_uncertainty_val, epoch_log)
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            async_save_model(state_dict, filename)
            if is_best:
                logger.info("Is best")
                async_save_model(state_dict, args.save_path + "/model/model_best.pth")
...
def train(
    ...
    for i, (
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            output, loss, uncertainty, kl_loss = model(
                support_offset,
                support_x,
                support_y,
                query_offset,
        ...
        uncertainty_meter.update(uncertainty, n)
        ...
        if main_process():
            ...
            # 删除 batch 级别的 uncertainty 和 kl_loss
            # writer.add_scalar("uncertainty_train_batch", uncertainty, current_iter)
            # writer.add_scalar("kl_loss_train_batch", kl_loss.item(), current_iter)
...
def validate(val_loader, model, valid_calsses):
    ...
    for i, batch in enumerate(val_loader):
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info(
                ...
                "KL {kl_loss:.4f} "
                "Accuracy {accuracy:.4f} "
                "Uncertainty {uncertainty_meter.val:.4f} ({uncertainty_meter.avg:.4f}).".format(
                    i + 1,
                    len(val_loader),
                    data_time=data_time,
                    batch_time=batch_time,
    ...
    return loss_meter.avg, mIoU, mAcc, allAcc, avg_uncertainty  # 新增


def async_save_model(state_dict, filename):
    def save_fn():
        torch.save(state_dict, filename)
    t = threading.Thread(target=save_fn)
    t.start()
